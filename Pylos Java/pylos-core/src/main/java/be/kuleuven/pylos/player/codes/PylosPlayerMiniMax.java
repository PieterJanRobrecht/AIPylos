...
public class PylosPlayerMiniMax extends PylosPlayer {
    ...
    private PylosSphere[] optionsSpheres = new PylosSphere[2];
    private ArrayList<PylosLocation>[] optionsLocations = new ArrayList[5];
    private ArrayList<PylosLocation> otherSquares = new ArrayList<>();
    private ArrayList<PylosLocation> mySquares = new ArrayList<>();
    private ArrayList<PylosLocation> moveUp = new ArrayList<>();
    private ArrayList<PylosLocation> reserveLocations = new ArrayList<>();

    private ArrayList<PylosLocation> dontKnow = new ArrayList<>();
    ...
    public void doMove(PylosGameIF game, PylosBoard board) {
        ...
        optionsLocations[4] = reserveLocations;

        //Check if there are any squares
        checkIfSquares(new ArrayList<>(Arrays.asList(locations)));
    ...
    private void resetArrays() {
        input = new int[5];
        boardOrReserve = new int[2];
        output = new int[5];
        choseSphere = new int[2];
        optionsSpheres = new PylosSphere[2];
        optionsLocations = new ArrayList[5];
        otherSquares = new ArrayList<>();
        mySquares = new ArrayList<>();
        moveUp = new ArrayList<>();
        reserveLocations = new ArrayList<>();
    }

    private void makeOutput() {
        // For the locations
        for(int i =0;i<optionsLocations.length;i++){
            if(optionsLocations[i].contains(bestLocation)){
                output[i] = 1;
                break;
            }else{
                output[i]=0;
        ...
        // For the spheres
        for (int i = 0; i < optionsSpheres.length; i++) {
            if (optionsSpheres[i] == null) {
                choseSphere[i] = 0;
            }else if (optionsSpheres[i].ID == bestSphere.ID) {
                choseSphere[i] = 1;
            } else {
                choseSphere[i] = 0;
            }
    ...
    private void checkIfSquares(ArrayList<PylosLocation> locations) {
        for(int i = 0; i< locations.size(); i++){
            if((locations.get(i).getMaxInSquare(this.OTHER) == 2 || locations.get(i).getMaxInSquare(this.OTHER) == 3) && locations.get(i).getSphere() == null){
                input[1] = 1;
                otherSquares.add(locations.get(i));
            }
            if ((locations.get(i).getMaxInSquare(this) == 2 || locations.get(i).getMaxInSquare(this) == 3)&& locations.get(i).getSphere() == null) {
                input[2] = 1;
                mySquares.add(locations.get(i));
            }
        }

        optionsLocations[1] = otherSquares;
        optionsLocations[2] = mySquares;
    }
    ...
    private void eval(double minimax, PylosSphere sphere, PylosLocation location) {
        ...
        if (minimax > bestMinimax) {
            ...
            if (sphere != null && optionsSpheres[1].ID != sphere.ID) {
                boardOrReserve[0] = 1;
                optionsSpheres[0] = sphere;
            }
        }
